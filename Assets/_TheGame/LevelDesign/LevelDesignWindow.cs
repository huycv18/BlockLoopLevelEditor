...
namespace BlockLoop.LevelDesign
{
    public class LevelDesignWindow : EditorWindow
    {
        ...
        // ── Level save/load ──
        const string LevelsFolderAssetPath = "Assets/_TheGame/Levels";
        const string ScreenshotsFolderAssetPath = "Assets/_TheGame/Screenshots";
        ...
        [MenuItem("Tools/Level/Level Design")]
        static void Open()
        {
            var w = GetWindow<LevelDesignWindow>("Level Design");
        ...
        void LoadPalette()
        {
            ...
            var cfg = Resources.Load<ColorConfigDataScriptableObject>("Config/ColorConfig");
            if (cfg == null)
            {
                var guids = AssetDatabase.FindAssets("t:ColorConfigDataScriptableObject");
                if (guids.Length > 0)
                    cfg = AssetDatabase.LoadAssetAtPath<ColorConfigDataScriptableObject>(AssetDatabase.GUIDToAssetPath(guids[0]));
            }
            if (cfg == null)
            {
        ...
        static string GetLevelsFolderAbsolutePath()
        {
            ...
            return Path.Combine(projectRoot, LevelsFolderAssetPath.Replace('/', Path.DirectorySeparatorChar));
        }
        ...
        {
            string projectRoot = Application.dataPath.Substring(0, Application.dataPath.Length - "Assets".Length);
            return Path.Combine(projectRoot, ScreenshotsFolderAssetPath.Replace('/', Path.DirectorySeparatorChar));
        }
